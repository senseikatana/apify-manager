 ...
 * ```
 */
export function useInterval(callback, ms, immediate = false) {
    let timerId = null;
    let isPaused = false;
    let isStopped = false;
 ...
 * ```
 */
export function useDebounce(func, delayMs) {
    let timeoutId;
    return (...args) => {
 ...
 * ```
 */
export function useDebounceImmediate(func, delayMs) {
    let timeoutId;
    let invoked = false;
    let lastArgs = null;
    return (...args) => {
 ...
 * ```
 */
export function useThrottle(func, limitMs) {
 ...
 * ```
 */
export function useThrottleTrailing(func, limitMs) {
    let inThrottle = false;
    let lastArgs = null;
    return (...args) => {
        if (!inThrottle) {
 ...
 * ```
 */
export async function useRepeat(callback, iterations, delayMs = 0) {
 ...
 * ```
 */
export async function useRace(promise, timeoutMs, errorMessage = "Operation timed out") {
    let timeoutId;
    let settled = false;

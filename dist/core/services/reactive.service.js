 ...
 * ```
 */
export function useCreateSignal(initialValue) {
 ...
 * ```
 */
export function useCreateEffect(callback, signals) {
    let cleanup;
    const execute = () => {
 ...
 * ```
 */
export function useCreateMemo(computation, signals) {
    const [get, set] = useCreateSignal(computation());
    useCreateEffect(() => {
        set(computation());
    }, signals);
    return get;
 ...
 * ```
 */
export function useCreateToggle(initialValue = false) {
    const [get, set] = useCreateSignal(initialValue);
 ...
 * ```
 */
export function useCreateStorageSignal(key, fallbackValue, target = "localStorage") {
    ...
    const [get, set] = useCreateSignal(initial);
 ...
 * ```
 */
export function useCreateDebouncedSignal(initialValue, delayMs = 300) {
    const [get, set] = useCreateSignal(initialValue);
    let timeoutId;

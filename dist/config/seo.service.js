import { siteConfig as defaultSiteConfig } from "./site.config.js";
/**
 ...
 * ```
 */
export function useSeoMeta(opts, defaults = defaultSiteConfig) {
    const { config, meta } = splitUseSeoMetaOptions(opts, defaults);
 ...
 */
export function useSeoTag(config, meta) {
...
function siteConfigToOpts(config) {
    return {
        site: config.site,
        siteTitle: config.title,
        lang: config.lang,
...
function splitUseSeoMetaOptions(opts, defaults) {
    ...
    const config = {
        ...
        title: brand,
        description: (typeof metaRest.description === "string" ? metaRest.description : undefined) ??
            defaults.description,
        lang: lang ?? defaults.lang,
        author: (typeof metaRest.author === "string" ? metaRest.author : undefined) ?? defaults.author,
        ogImage: typeof metaRest.ogImage === "string" ? metaRest.ogImage : (defaults.ogImage ?? undefined),
 ...
 */
export function useApplySeoTag(seo, head = typeof document !== "undefined" ? document.head : null) {
...
function isLegacySeoMeta(meta) {
    return ("publishedTime" in meta ||
        "modifiedTime" in meta ||
        "noindex" in meta ||
        ("tags" in meta && !("articleTag" in meta)));
}
// --- Merge & legacy ---
function legacySeoMetaToInput(meta) {
    const input = {
        title: meta.title,
    };
    ...
    if (meta.canonical != null)
        input.canonical = meta.canonical;
    if (meta.url != null) {
        input.url = meta.url;
        input.ogUrl = meta.url;
    }
    if (meta.ogImage != null)
        input.ogImage = meta.ogImage;
    if (meta.ogType != null)
        input.ogType = meta.ogType;
    if (meta.author != null)
    ...
    if (meta.ogType === "article") {
        ...
        if (meta.author)
            input.articleAuthor = [meta.author];
    }
    if (meta.noindex) {
...
function mergeSeoDefaults(input, config) {
    if (!config)
        return { ...input };
    const pageTitle = input.title ?? config.title;
    const siteSuffix = ` | ${config.title}`;
    const alreadySuffixed = typeof pageTitle === "string" && pageTitle !== config.title && pageTitle.endsWith(siteSuffix);
    const title = pageTitle === config.title || !pageTitle || alreadySuffixed
...
function resolveOgImageUrl(input, config) {
...
function flattenSeoMetaToTags(input, config) {
    const tags = [];
    if (input.charset) {
    ...
    for (const [key, raw] of Object.entries(input)) {
        ...
        if (key === "robots") {
            const content = serializeRobots(raw);
            if (content)
                tags.push({ tag: "meta", attrs: { name: "robots", content } });
            continue;
        }
        if (key === "viewport" && typeof raw === "object") {
            const content = Object.entries(raw)
                .filter(([, v]) => v != null)
                .map(([k, v]) => `${camelToKebab(k)}=${v}`)
                .join(", ");
            if (content)
                tags.push({ tag: "meta", attrs: { name: "viewport", content } });
            continue;
        }
...
function pushScalarMeta(tags, key, value) {
...
function flattenOgImage(raw, config) {
    const tags = [];
    const prop = "og:image";
    if (typeof raw === "string") {
        tags.push({
...
function serializeRobots(value) {
...
function camelToMetaName(key) {
    return camelToKebab(key);
}
function camelToMetaProperty(key) {
    const overrides = {
...
function buildRssTagNode(config) {
...
function buildJsonLdFromInput(input, config) {
...
function serializeSeoTags(tags) {
    return tags.map(serializeSeoTag).join("\n");
}
function serializeSeoTag(node) {
    if (node.tag === "title") {

 ...
 * ```
 */
export function useObserverCreate(key, callback, options = { threshold: 0.1 }, autoUnobserve = true) {
    if (!useIsSupported()) {
        ...
        for (const entry of entries) {
            if (entry.isIntersecting) {
                ...
                if (config.autoUnobserve) {
                    useObserverUnobserve(key, entry.target);
                }
            }
        }
    }, config.options);
 ...
 * ```
 */
export function useObserverObserve(key, element) {
 ...
 * ```
 */
export function useObserverObserveAll(key, selector) {
    if (!useIsSupported())
        return;
 ...
 */
export function useObserverUnobserve(key, element) {
    const entry = registry.get(key);
    if (!entry?.observer)
        return;
 ...
 * ```
 */
export function useObserverDisconnect(key) {
    const entry = registry.get(key);
    if (!entry)
        return;
 ...
 * ```
 */
export function useObserverDisconnectAll() {
 ...
 */
export function useObserverHas(key) {
 ...
 */
export function useObserverKeys() {
 ...
 * ```
 */
export function useLazyLoaderInit(key = "default", selector = "img[data-src]", rootMargin = "200px") {
    if (!useIsSupported())
        return;
    ...
    useObserverCreate(observerKey, (entry) => {
 ...
 */
export function useLazyLoaderStop(key) {
 ...
 */
export function useLazyLoaderStopAll() {
 ...
 */
export function useLazyLoaderHas(key) {
